...
import { BeneficiaryListItem, BeneficiaryListItemData } from './beneficiary-list-item.po';
...
export class BatchBeneficiarySelector {
  ...
  async selectBeneficiary(beneficiaryName: string | null, beneficiaryType: BeneficiaryType): Promise<void> {
    ...
    await this.dropdownMenuPanel
      .locator(`${BeneficiaryListItem.beneficiaryNameLocator}:has-text('${beneficiaryName}')`)
      .click();
  }
  ...
  async getLoadedData(beneficiaryType: BeneficiaryType): Promise<BeneficiaryListItemData[]> {
    ...
    for (let i = 0; i < (await loadedBeneficiaryListItems.count()); i++) {
      const listItem = new BeneficiaryListItem(loadedBeneficiaryListItems.nth(i));
